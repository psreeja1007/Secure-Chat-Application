...
static bool send_client_message(int client_fd, const aesgcm::Key &key, const string &message) {
    lock_guard<mutex> lock(send_mutex);
    return aesgcm::send_encrypted(client_fd, key, message);
}

static bool is_e2e_control(const string &message) {
    return message.rfind("__E2E_INIT__", 0) == 0 ||
           message.rfind("__E2E_ACK__", 0) == 0 ||
           message.rfind("__E2E_MSG__", 0) == 0;
}

static string relay_payload(const string &sender, const string &message) {
    if (!is_e2e_control(message)) return "You received this from client " + sender + ": " + message + "\n";

    string prefix;
    if (message.rfind("__E2E_INIT__", 0) == 0) prefix = "__E2E_INIT__";
    else if (message.rfind("__E2E_ACK__", 0) == 0) prefix = "__E2E_ACK__";
    else prefix = "__E2E_MSG__";
    return prefix + sender + "|" + message.substr(prefix.size());
}
...
static void handle_client(int client_fd, const string &client_ip) {
    ...
    if (!send_client_message(client_fd, key, "Hello from server. Please register.\n")) {
        close(client_fd);
        return;
    }
    ...
    while (!registered) {
        ...
        if (message.rfind(prefix, 0) != 0) {
            send_client_message(client_fd, key, "Invalid format. Please use: #register <username>\n");
            continue;
        }

        username = message.substr(prefix.length());
        if (username.empty()) {
            send_client_message(client_fd, key, "Username cannot be empty. Please use: #register <username>\n");
            continue;
        }

        {
            ...
            if (client_sockets.find(username) != client_sockets.end()) {
                send_client_message(client_fd, key, "Username already taken. Select another one.\n");
                continue;
            }
        ...
        send_client_message(client_fd, key, "Successfully registered as " + username + "\n");
    ...
    while (true) {
        ...
        if (message == "#register" || message.rfind("#register ", 0) == 0) {
            send_client_message(client_fd, key,
                "Error: you are already registered as " + username + ".\n");
            continue;
        }
        ...
        if (!message.empty() && message[0] == '@') {
            ...
            if (space_pos != string::npos) {
                ...
                if (!is_e2e_control(actual_message)) {
                    send_client_message(client_fd, key,
                        "Error: end-to-end encryption is required for chat messages.\n");
                    continue;
                }
                ...
                if (target_fd == -1) {
                    send_client_message(client_fd, key, "User " + target_username + " is not online.\n");
                    continue;
                }

                send_client_message(target_fd, target_key,
                    relay_payload(username, actual_message));
        ...
        if (message == "/who") {
            ...
            send_client_message(client_fd, key, reply);
            continue;
        }

        if (message == "/quit") {
            send_client_message(client_fd, key, "Goodbye!\n");
            break;
        }
        ...
        if (target_username.empty()) {
            send_client_message(client_fd, key, "No chatting partner selected. Use /chat <username> first.\n");
            continue;
        }
        ...
        if (target_fd == -1) {
            send_client_message(client_fd, key, "User " + target_username + " is no longer online.\n");
            continue;
        }
