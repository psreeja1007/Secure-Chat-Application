...
static bool perform_dh_server(int client_fd, const string &client_ip, aesgcm::Key &key_out) {
    ...
    try {
        ...
        string server_handshake_message = "DH_INIT " + dh::bn_to_hex(key_pair.pub) + "\n";
        if (!send_plain(client_fd, server_handshake_message)) {
            perror("DH init send failed");
        ...
        string client_reply;
        if (!recv_plain_line(client_fd, client_reply)) {
            cout << "Client disconnected during DH handshake: " << client_ip << endl;
            dh::free_keypair(key_pair);
            BN_CTX_free(bignum_context);
            return false;
        }

        if (client_reply.rfind("DH_RESP ", 0) != 0) {
            cout << "Invalid DH response from client " << client_ip << ": " << client_reply << endl;
            dh::free_keypair(key_pair);
            BN_CTX_free(bignum_context);
            return false;
        }

        string peer_public_key_hex = client_reply.substr(8);
        BIGNUM *peer_public_key = nullptr;
        if (peer_public_key_hex.empty() || !dh::hex_to_bn(&peer_public_key, peer_public_key_hex)) {
            cout << "Malformed client DH public value from " << client_ip << endl;
            dh::free_keypair(key_pair);
            BN_CTX_free(bignum_context);
            return false;
        }

        BIGNUM *shared_secret = dh::compute_shared_secret(key_pair, peer_public_key, bignum_context);
