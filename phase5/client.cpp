...
constexpr uint64_t E2E_KEY_LIFETIME_SECONDS = 60;
...
static bool e2e_key_expired(const E2ESession &session) {
    return session.established &&
           current_timestamp() >= session.timestamp + E2E_KEY_LIFETIME_SECONDS;
}

static void print_e2e_key(const string &peer, const aesgcm::Key &key, uint64_t timestamp) {
    cout << "[E2E] Key with client " << peer
         << " | fingerprint: " << dh::fingerprint_hex(key)
...
static bool handle_e2e_control(int sock_fd, const aesgcm::Key &server_key,
    ...
    string public_hex = data.substr(data_separator + 1);
    BIGNUM *peer_public = nullptr;
    if (!dh::hex_to_bn(&peer_public, public_hex)) return true;
    BN_CTX *ctx = BN_CTX_new();
    ...
    bool accepted = false;
    {
        ...
        if (send_ack && session.established && !e2e_key_expired(session) &&
            session.pending_keypair.pub) {
        ...
        if (!accepted && send_ack) {
            ...
            bool same_key = session.pending_keypair.pub &&
                session.pending_timestamp == exchange_timestamp &&
                dh::bn_to_hex(session.pending_keypair.pub) == public_hex;

            if (local_wins && !same_key) {
                BN_free(peer_public);
                BN_CTX_free(ctx);
                return true;
            }

            if (!session.pending_keypair.pub || (!local_wins && !same_key)) {
                dh::free_keypair(session.pending_keypair);
                session.pending_keypair = dh::generate_keypair(ctx);
            }
            session.pending_timestamp = exchange_timestamp;
        ...
        if (!accepted || !(send_ack && session.established)) {
    ...
    print_e2e_key(peer, established_key, established_timestamp);
